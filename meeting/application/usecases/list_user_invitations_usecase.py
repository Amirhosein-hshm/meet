...
@dataclass
class ListUserInvitationsRequestInput:
    actor_id: int
    actor_role: Role
    page: int = 1
    size: int = 20
...
@dataclass
class ListUserInvitationsItemData:
    id: int
    title: str
    meet_hash: str
    start_time: datetime
...
@dataclass
class ListUserInvitationsResponseOutput:
    items: List[ListUserInvitationsItemData]
    total: int
    page: int
    size: int
...
class ListUserInvitationsUseCase:
    ...
    def execute(self, request: ListUserInvitationsRequestInput) -> ListUserInvitationsResponseOutput:
        filters = MeetFilter(
            actor_id=request.actor_id,
            actor_role=request.actor_role,
            participant_user_id=request.actor_id,
        ...
        return ListUserInvitationsResponseOutput(
            items=items,
            total=total,
            page=request.page,
            size=request.size,
